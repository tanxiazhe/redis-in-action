...
def article_vote(conn, user, article, votes='up'):
    ...
    if conn.sadd('voted:' + article_id, user):  # E
        ...
        if votes is 'up':
            conn.hincrby(article, 'upvotes', 1)  # E
        elif votes is 'down':
...
class TestCh01(unittest.TestCase):
    ...
    def test_article_functionality(self):
        ...
        print("We up voted for the article, it now has up votes:", end=' ')
        v = int(conn.hget('article:' + article_id, 'upvotes'))
        print(v)
        print()
        self.assertTrue(v > 1)

        article_vote(conn, 'other_user', 'article:' + article_id, 'down')
        print("We down voted for the article, it now has down votes:", end=' ')
        v = int(conn.hget('article:' + article_id, 'downvotes'))
        print(v)
        print()
        self.assertTrue(v == 0)

        article_vote(conn, 'other_user_2', 'article:' + article_id, 'down')
        print("We down voted for the article, it now has down votes:", end=' ')
        v = int(conn.hget('article:' + article_id, 'downvotes'))
        print(v)
        print()
